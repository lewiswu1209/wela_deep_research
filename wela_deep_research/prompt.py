...
- Use same language as the report topic, e.g. if the report topic is in Chinese, use Chinese for section names and descriptions

Before submitting, review your structure to ensure it has no redundant sections and follows a logical flow.
</Task>

You should return a JSON array containing section objects, where each object must include the following fields:
- Name: str - Name for this section of the report.
- Description: str - Brief overview of the main topics covered in this section.
- Research: bool - Whether to perform web research for this section of the report.
- Content: str - The content of the section, which you will leave blank for now.

today is {today}."""

queries_generator_instructions = """You are an expert technical writer crafting targeted web search queries that will gather comprehensive information for writing a technical report section.
...
3. IMPORTANT!!! Each query contains a series of keywords that can be directly used in search engines
4. Use specific time expressions instead of vague ones
5. Use same language as the report topic, e.g. if the report topic is in Chinese, use Chinese for section names and descriptions

Make the queries specific enough to find high-quality, relevant sources.
</Task>

You should return a JSON Array of queries, each query is a str.
...
2. If present, review any existing section content and feedback. 
...
8. Use same language as the report topic, e.g. if the report topic is in Chinese, use Chinese for section names and descriptions
...
1. Each query contains a series of keywords that can be directly used in search engines
2. Use specific time expressions instead of vague ones
3. Use same language as the report topic, e.g. if the report topic is in Chinese, use Chinese for section names and descriptions

Make the queries specific enough to find high-quality, relevant sources.
</task>

<format>
Output the JSON object with following schema:
```json
{{
"grade": "Evaluation result indicating whether the response meets requirements ('pass') or needs revision ('fail').",
"feedback": "If the evaluation result is 'fail', provide feedback on how to revise this section.",
follow_up_keywords: [
    "First query to gather missing information",
    "Second query to gather missing information",
    "Third query to gather missing information",
    ...Add more queries as needed, up to a maximum of {number_of_follow_up_queries}
]
}}
```
</format>"""

conclusion_section_writer_instructions = """You are an expert technical writer crafting a section that synthesizes information from the rest of the report.
...
Introduction_section_writer_instructions = """You are an expert technical writer crafting a section that synthesizes information from the rest of the report.
